<?php
// LIMA Solutions ERP - Quotes Create & Edit Form View
...
enforceModuleAccess('quotes', $userRole, $companyId, 'edit', $pdo);

$quote = null;
$id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if ($id > 0) {
    // Edit mode: fetch quote from database enforcing company isolation
    $stmt = $pdo->prepare("SELECT * FROM quotes WHERE id = :id AND company_id = :company_id AND deleted_at IS NULL LIMIT 1");
...
<head>
    ...
    <title><?php echo $quote ? 'Modifier' : 'Nouveau'; ?> Devis - LIMA Solutions</title>
    <!-- Google Fonts -->
        ...
        <main class="quotes-container">
            <div class="quotes-header-row">
                <div class="quotes-title-section">
                    <h2><?php echo $quote ? 'Modifier' : 'Nouveau'; ?> Orçamento / Devis</h2>
                    <p><?php echo $quote ? 'Mettre à jour les informations du devis.' : 'Rédiger une offre commerciale.'; ?></p>
                </div>
            </div>

            <form id="quotes-form">
                <?php if ($quote): ?>
                    <input type="hidden" id="quote-id" value="<?php echo (int)$quote['id']; ?>">
                <?php endif; ?>

                <!-- Row 1: Client & General Info -->
                <div class="quotes-card" style="margin-bottom: 24px;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
                        <div class="crm-form-group">
                            ...
                            <select id="client_id" required data-selected="<?php echo htmlspecialchars($quote['client_id'] ?? ''); ?>">
                        ...
                        <div class="crm-form-group">
                            ...
                            <input type="date" id="issue_date" required value="<?php echo htmlspecialchars($quote['issue_date'] ?? date('Y-m-d')); ?>">
                        </div>
                        <div class="crm-form-group">
                            <label for="valid_until">Valable jusqu'au *</label>
                            <input type="date" id="valid_until" required value="<?php echo htmlspecialchars($quote['valid_until'] ?? date('Y-m-d', strtotime('+30 days'))); ?>">
                        </div>
                    </div>
                </div>
                ...
                <div class="quotes-card" style="margin-bottom: 24px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-gray); padding-bottom: 10px; margin-bottom: 15px;">
                        <h3 style="font-size: 15px; font-weight: 700; color: var(--primary-teal-dark); margin: 0;"><i class="fa-solid fa-list-ol"></i> Lignes de devis</h3>
                        ...
                        <table class="summary-table">
                            ...
                            <tr>
                                ...
                                <td style="text-align: right; width: 150px;">
                                    <input type="number" id="discount_percent" step="0.01" min="0.00" max="100.00" style="width: 80px; padding: 6px; border: 1px solid var(--border-gray); border-radius: var(--border-radius); text-align: right;" value="<?php echo htmlspecialchars($quote['discount_percent'] ?? '0.00'); ?>">
                                </td>
                            </tr>
                ...
                <div class="quotes-card" style="margin-bottom: 24px;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                        <div class="crm-form-group">
                            ...
                            <textarea id="notes" rows="4" placeholder="Ex: Conditions de paiement, modalités de livraison..."><?php echo htmlspecialchars($quote['notes'] ?? ''); ?></textarea>
                        </div>
                        <div class="crm-form-group">
                            <label for="internal_notes">Notes internes (confidentiel)</label>
                            <textarea id="internal_notes" rows="4" placeholder="Saisir des notes internes uniquement visibles par le staff..."><?php echo htmlspecialchars($quote['internal_notes'] ?? ''); ?></textarea>
                        </div>
                    </div>
                </div>

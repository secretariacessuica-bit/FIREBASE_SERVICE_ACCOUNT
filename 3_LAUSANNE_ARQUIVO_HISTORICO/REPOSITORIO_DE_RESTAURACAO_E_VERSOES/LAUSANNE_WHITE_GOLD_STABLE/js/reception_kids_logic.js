...
window.loadAllKidsList = async function() {
    ...
    listEl.innerHTML = '<div style="color:#aaa; text-align:center; padding:20px;"><i class="fas fa-spinner fa-spin"></i> Carregando lista...</div>';
    ...
    try {
        ...
        listEl.innerHTML = '<div style="color:red; padding:10px;">Erro ao carregar lista de crianças.</div>';
    }
}
...
window.renderKidsSelection = function(filter = '') {
    ...
    const filtered = allKidsCache.filter(k =>
        ...
        (k.parent && k.parent.toLowerCase().includes(query))
    );

    if (filtered.length === 0) {
        listEl.innerHTML = '<div style="color:#aaa; text-align:center; padding:20px;">Nenhuma criança encontrada.</div>';
        return;
    }
    ...
    filtered.forEach(k => {
        ...
        div.style.cssText = "background:#ffffff; padding:15px; margin-bottom:12px; border-radius:12px; cursor:pointer; border:1px solid #e00; box-shadow:0 4px 10px rgba(0,0,0,0.05); transition: 0.2s; border:1px solid #eee;";
        
        // Ensure we use the correct fields. Some old records might use different ones?
        // Let's be safe and check common patterns.
...
// 3. CHECK-IN EXISTING KID
window.submitChildExisting = function (kidData) {
    if (!confirm(`Confirma check-in de ${kidData.name}?`)) return;
    ...
    db.collection('attendance').add({
        ...
        alert("Check-in enviado! Aguardando confirmacao na Sala das Criancas.");
        document.getElementById('child-search-input').value = '';
        document.getElementById('child-search-results').innerHTML = '';
